...
class OrdersAcceptedController extends GetxController {
 ...
 AcceptedOrdersData orderData = AcceptedOrdersData(Get.find());
  ...
  getOrders() async {
    ...
    update();
    var response = await orderData.getData(myServices.sharedPreferences.getString("id")!);
...
donedDelivery(String ordersId,String userid) async {
    data.clear();
    statusRequest = StatusRequest.loading;
    update();
    var response = await orderData.doneDelevary(ordersId,userid);
    statusRequest = handlingData(response);
    if(StatusRequest.success == statusRequest){
      if(response['status'] == "success"){
          getOrders();
